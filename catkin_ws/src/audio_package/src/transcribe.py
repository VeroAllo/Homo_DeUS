...
class TranscriberThread(threading.Thread):
    ...
    def get_microphone(self):
        ...
        print('Using default microphone:', device_info['name'])
        stream = sd.InputStream(callback=self.callback, samplerate=16000, channels=1)
        stream.start()
        return stream
    ...
    def record_callback(self, indata, frames, time, status):
        data = indata.tobytes()
        data_base64 = base64.b64encode(data).decode()  # Convertir les bytes en base64
        self.data_queue.put(data_base64)

    def callback(self, indata, frames, time, status):
        # Convert the audio data to bytes, then to base64
        audio_data = indata.astype(np.int16).tolist()
        self.data_queue.put(audio_data)
        sleep(0.1)  # Pause for a short time to allow the transcribe method to process the data
            ...
            while True:
                try:
                    ...
                    if not self.data_queue.empty():
                        ...
                        # Traiter les données audio comme une liste d'entiers
                        audio_data = [item for sublist in self.data_queue.queue for item in sublist]
                        ...
                        sleep(0.25)
                        torch.cuda.empty_cache() # Libérer la mémoire de la carte graphique
